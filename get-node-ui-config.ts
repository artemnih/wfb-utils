import { PluginX, NodeInput, NodeInputUI, UIConfig } from '@polus-wfb/common';
import { isDirectory } from './is-dir';

const configMap = new Map<string, UIConfig>();
...
export function getNodeUiConfig(plugin: PluginX): UIConfig {
    ...
    if (configMap.has(plugin.pid)) {
        return configMap.get(plugin.pid) as UIConfig;
    }
    ...
    const result = { ui: uiInputs, inputs: nonUiInputs, outputs: outputs };

    // cache the result
    configMap.set(plugin.pid, result);
